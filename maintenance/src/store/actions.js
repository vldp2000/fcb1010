...
// ---addNewSong-------
export async function addNewSong (getters, song) {
...
const actions = {
  ...
  async addSong ({ commit, getters }, song) {
    await addNewSong(getters, song)
  ...
  async updateInstrument ({ commit }, instrument) {
    await InstrumentsService.put(instrument)
  ...
  async updateInstrumentBank ({ commit }, instrumentBank) {
    await InstrumentBankService.put(instrumentBank)
  ...
  sendChangePresetVolumeMessage ({ commit }, payload) {
    this._vm.$socket.client.emit(config.controllerPresetVoluleMessage, payload)
  },
