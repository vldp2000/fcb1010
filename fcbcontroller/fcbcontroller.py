...
def setPreset(program, songPreset, idx):
    ...
    if preset:
        ...
        if newPC == 0:  # mute the channel for PC=0
            ...
            printDebug(f"Preset Volume {newVolume}")
            #newVolume = newVolume + \
            #    int(newVolume * displayData.g_VolumeAmplify *
            #        VOLUME_AMPLIFY_PERCENT / 100)
...
def muteChannel(channel, volume, delay, step):
    printDebug(f"<<<  Mute channel {channel}")
    if volume > 0:
        x = volume
        while x > 0:
...
def unmuteChannel(channel, volume, delay, step):
    printDebug(f" >>  Unmute channel {channel}")
    x = step
    while x < volume:
        sendCCMessage(channel, VOLUME_CC, x)
        x = x + step
...
def sendPedalVolumeCC(channel, idx, volume):
    ...
    if volume <= maxVol:
        # channel 1  sendCCMessage(channel, 7, volume) #channel 1
        sendCCMessage(channel, 7, volume)
        printDebug(
            f" Pedal Volume ->  channel={channel},  maxVolume={maxVol} volume={volume} ")
